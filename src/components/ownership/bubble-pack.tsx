...
function companyTooltipHtml(d: d3.HierarchyNode<BubbleNode>): string {
  ...
  const color =
    CLUSTER_COLORS[(d.parent?.data.cluster ?? 0) % CLUSTER_COLORS.length];
  ...
  return `
    <div style="font-family:var(--font-mono,monospace)">
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <strong>${id}</strong>
        <span style="color:#72857e;font-size:10px">${clusterName}</span>
      </div>
      <div style="color:#b8c3be;font-size:10px;margin:2px 0">${name}</div>
      <div style="border-top:1px solid #233534;margin:4px 0"></div>
      <div style="display:flex;gap:12px;font-size:10px">
        <div><span style="color:#72857e">Insiders</span> <strong>${size}</strong></div>
        <div><span style="color:#72857e">Holding</span> <strong>${pct}%</strong></div>
        <div><span style="color:#72857e">Shared</span> <strong>${sharedInvestors ?? 0}</strong></div>
      </div>
      <div style="color:#72857e;font-size:9px;margin-top:4px">Click to view company</div>
    </div>`;
}

function clusterTooltipHtml(d: d3.HierarchyNode<BubbleNode>): string {
  const { name, memberCount, totalClusterInvestors, avgClusterPct, children } =
    d.data;
  const color = CLUSTER_COLORS[d.data.cluster % CLUSTER_COLORS.length];
  const topCompanies = (children ?? [])
  ...
  return `
    <div style="font-family:var(--font-mono,monospace)">
      ...
      <div style="border-top:1px solid #233534;margin:4px 0"></div>
      <div style="display:flex;gap:12px;font-size:10px">
        <div><span style="color:#72857e">Companies</span> <strong>${memberCount ?? 0}</strong></div>
        <div><span style="color:#72857e">Investors</span> <strong>${totalClusterInvestors ?? 0}</strong></div>
        <div><span style="color:#72857e">Avg %</span> <strong>${avgClusterPct?.toFixed(1) ?? "0"}%</strong></div>
      </div>
      ${
        topCompanies.length > 0
          ? `
        <div style="border-top:1px solid #233534;margin:4px 0"></div>
        <div style="color:#72857e;font-size:9px;margin-bottom:2px">Top Companies</div>
        ${topCompanies.map((c) => `<div style="font-size:10px"><strong>${c.id}</strong> <span style="color:#72857e">${c.name}</span></div>`).join("")}
      `
          : ""
      }
    </div>`;
...
export function BubblePack({ data, onCompanyClick }: BubblePackProps) {
  ...
  useEffect(() => {
    ...
    node
      ...
      .attr(
        "fill",
        (d) => CLUSTER_COLORS[d.data.cluster % CLUSTER_COLORS.length],
      )
