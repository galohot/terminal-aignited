...
export function ChordDiagram({ data, onArcClick, onRibbonClick }: ChordDiagramProps) {
	...
	useEffect(() => {
		...
		g.selectAll(".ribbon")
			...
			.on("mouseenter", function (event, d) {
				g.selectAll(".ribbon").transition().duration(200).style("opacity", 0.06);
				...
				tooltip
					...
					.html(
						`<strong>${data.names[d.source.index]}</strong> ↔ <strong>${data.names[d.target.index]}</strong><br/>` +
							`<span style="color:#55598a">Co-investments:</span> ${d.source.value}`,
					);
			})
			.on("mouseleave", () => {
				g.selectAll(".ribbon").transition().duration(200).style("opacity", 0.35);
		...
		// Arcs
		const groups = g.selectAll(".arc").data(chords.groups).enter().append("g").attr("class", "arc");
		...
		groups
			...
			.on("mouseenter", function (event, d) {
				...
				g.selectAll(".ribbon")
					...
					.duration(200)
					.style("opacity", (r: any) =>
						r.source.index === d.index || r.target.index === d.index ? 0.7 : 0.04,
					);
				tooltip
					.style("display", "block")
					.style("left", `${event.offsetX + 14}px`)
