...
export function Heatmap({ data }: HeatmapProps) {
	...
	useEffect(() => {
		...
		const cellSize = Math.max(
			13,
			Math.min(22, (width - margin.left - margin.right) / data.companies.length),
		);
		...
		// Light-theme color scale: paper (zero) -> ember (high)
		const colorScale = (pct: number) => {
			if (pct <= 0) return "#ece6d6";
			if (pct <= 2) return d3.interpolateRgb("#ece6d6", "#ffe0bf")(pct / 2);
			if (pct <= 10) return d3.interpolateRgb("#ffe0bf", "#ffbf8a")((pct - 2) / 8);
			if (pct <= 30) return d3.interpolateRgb("#ffbf8a", "#ff8a2a")((pct - 10) / 20);
			return d3.interpolateRgb("#ff8a2a", "#ff6a0a")(Math.min((pct - 30) / 30, 1));
		};
		...
		for (let ri = 0; ri < data.investors.length; ri++) {
			for (let ci = 0; ci < data.companies.length; ci++) {
				const pct = cellMap.get(`${data.investors[ri]}|${data.companies[ci].code}`) ?? 0;
				g.append("rect")
					...
					.attr("stroke", pct > 0 ? "rgba(255,138,42,0.35)" : "none")
						...
						tooltip
							...
							.html(
								`<strong>${truncate(data.investors[ri], 30)}</strong><br/>` +
									`holds <strong style="color:#ff6a0a">${pct.toFixed(2)}%</strong> of<br/>` +
					...
					.on("mouseleave", function () {
						...
						g.selectAll(".row-label").attr("fill", "#55598a").attr("font-weight", "500");
						g.selectAll(".col-label").attr("fill", "#55598a").attr("font-weight", "500");
		...
		const legendGrad = defs.append("linearGradient").attr("id", "heatmap-legend-grad");
