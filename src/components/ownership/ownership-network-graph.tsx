...
function nodeRadius(d: SimNode) {
	return d.type === "investor" ? Math.sqrt(d.size) * 3 + 5 : Math.sqrt(d.size) * 2.5 + 4;
}

function nodeColor(d: SimNode) {
	if (d.type === "investor" && d.investorType) return INVESTOR_TYPE_COLORS[d.investorType];
	return "#55598a";
}
...
export function NetworkGraph({ data, onNodeClick }: NetworkGraphProps) {
	...
	const drawGraph = useCallback(() => {
		...
		const linkG = g
			...
			.attr("fill", "none")
			.attr("stroke", "#d8cfb9")
			.attr("stroke-width", (d) => Math.max(1, Math.sqrt(d.percentage) * 0.8))
			...
			.on("mouseenter", function (event, d) {
				...
				const typeLabel =
					d.type === "investor" && d.investorType
						...
						: "Company";
				const originLabel = d.localForeign ? (LOCAL_FOREIGN_LABELS[d.localForeign] ?? "") : "";
	...
	return (
		<div
			ref={containerRef}
			className="relative w-full overflow-hidden rounded-[18px] border border-rule bg-card"
				...
				<div className="absolute inset-0 flex items-center justify-center">
					<div className="text-center space-y-2">
						<svg
							...
							stroke="currentColor"
							strokeWidth="1.5"
							className="mx-auto text-ink-4/50"
						>
						...
						<p className="font-mono text-xs text-ink-4/70">
							Try adjusting the investor type, origin, or lowering the minimum connections
						</p>
					</div>
				</div>
			)}
