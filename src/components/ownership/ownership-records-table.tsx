...
export function RecordsTable({
  ...
  function handleSort(key: SortKey) {
    if (sortKey === key) {
      ...
      setSortKey(key);
      setSortDir(
        key === "PERCENTAGE" || key === "TOTAL_HOLDING_SHARES"
          ? "desc"
          : "asc",
      );
    }
  ...
  const hasFilters =
    query || selectedTypes.length > 0 || selectedLF.length > 0;
  ...
  return (
    <div className="overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <h3 className="font-mono text-[13px] font-semibold text-t-text tracking-tight">
            All Holdings
          </h3>
          <p className="font-mono text-[11px] text-t-text-muted mt-0.5">
        ...
        {hasFilters && (
          <button
            ...
            className="font-mono text-[11px] font-medium text-t-amber hover:text-t-amber/80 transition-colors flex items-center gap-1"
          >
            <svg
              width="12"
              height="12"
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <svg
            className="absolute left-3.5 top-1/2 -translate-y-1/2 text-t-text-muted"
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
          ...
          <input
            ...
            className="w-full rounded-lg border border-t-border bg-t-surface pl-10 pr-4 py-2 font-mono text-sm text-t-text placeholder:text-t-text-muted/70 focus:outline-none focus:ring-2 focus:ring-t-amber/20 focus:border-t-amber/40 transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
              ...
              <button
                ...
                className={clsx(
                  ...
                  selected
                    ...
                    : "border-t-border text-t-text-muted hover:text-t-text hover:border-t-border hover:bg-white/[0.04]",
                )}
                style={
                  selected
                    ? {
          ...
          <span className="w-px h-6 bg-t-border self-center mx-1.5" />
            ...
            <button
              ...
              className={clsx(
                ...
                selectedLF.includes(v)
                  ? "bg-t-amber/10 text-t-amber border-t-amber/20 shadow-sm"
                  : "border-t-border text-t-text-muted hover:text-t-text hover:border-t-border hover:bg-white/[0.04]",
              )}
            >
            ...
            <tr className="border-b border-t-border">
              {COLUMNS.map((col) => (
                <th
                  ...
                  className={clsx(
                    "text-left font-mono text-[11px] font-semibold text-t-text-muted uppercase tracking-wider px-2 py-2 cursor-pointer select-none",
                    col.className,
                  )}
                >
                  <span className="inline-flex items-center gap-1">
                    ...
                    {sortKey === col.key && (
                      <svg
                        ...
                        strokeWidth="2.5"
                        className={clsx(
                          "transition-transform text-t-amber",
          ...
          <tbody>
            {pageRows.map((r, i) => (
              <tr
                ...
                className="border-b border-t-border/50 hover:bg-white/[0.02] transition-colors"
              >
                <td className="px-2 py-1.5">
                  <Link
                    to={`/idx/${r.SHARE_CODE}`}
                    className="font-mono font-bold text-t-amber hover:underline underline-offset-2"
                  >
                    {r.SHARE_CODE}
                  </Link>
                </td>
                <td className="px-2 py-1.5 max-w-[200px]">
                  <Link
                    to={`/idx/${r.SHARE_CODE}`}
                    className="font-mono text-[12px] text-t-text truncate block hover:text-t-amber transition-colors"
                    title={r.ISSUER_NAME}
                  >
                    {r.ISSUER_NAME}
                  </Link>
                ...
                <td className="px-2 py-1.5 font-mono text-[12px] text-t-text-muted hidden xl:table-cell">
                  {r.NATIONALITY || "\u2014"}
                </td>
                <td className="px-2 py-1.5 font-mono text-[12px] text-t-text-muted hidden lg:table-cell">
                  {r.DOMICILE || "\u2014"}
                </td>
                <td className="px-2 py-1.5 text-right font-mono text-[12px] text-t-text-muted tabular-nums">
                  {formatShares(r.TOTAL_HOLDING_SHARES)}
                </td>
                <td className="px-2 py-1.5 text-right font-mono text-[12px] font-bold text-t-text tabular-nums">
            ...
            {pageRows.length === 0 && (
              <tr>
                <td
                  ...
                  className="!py-12 text-center text-t-text-muted"
                >
                  <div className="flex flex-col items-center gap-2">
                    <svg
                      ...
                      stroke="currentColor"
                      strokeWidth="1.5"
                      className="text-t-text-muted/40"
                    >
                      <circle cx="11" cy="11" r="8" />
                      <line x1="21" y1="21" x2="16.65" y2="16.65" />
                    </svg>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between py-3 mt-2">
          <p className="font-mono text-[11px] text-t-text-muted font-medium">
            <span className="font-mono tabular-nums">
              {fmtNum(safePage * pageSize + 1)}
            </span>
          ...
          <div className="flex items-center gap-0.5">
            <button
              ...
              onClick={() => setPage(0)}
              disabled={safePage === 0}
              className="p-1 rounded text-t-text-muted hover:text-t-text hover:bg-white/[0.08] disabled:opacity-30 disabled:pointer-events-none transition-colors"
              title="First page"
            >
              <svg
                width="14"
            ...
            <span className="px-3 py-1 font-mono text-xs font-semibold tabular-nums text-t-text">
              {safePage + 1}
              <span className="text-t-text-muted font-normal mx-0.5">
                /
              </span>
              {totalPages}
            </span>
