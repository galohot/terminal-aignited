...
export function SankeyDiagram({ data, onNodeClick, onLinkClick }: SankeyDiagramProps) {
	...
	useEffect(() => {
		...
		const leftNodes = Array.from(nodeMap.values()).filter((n) => n.id.startsWith("src:"));
		const rightNodes = Array.from(nodeMap.values()).filter((n) => n.id.startsWith("cat:"));
		...
		for (const node of rightNodes) {
			let ty = node.y0!;
			const nodeH = node.y1! - node.y0! || 1;
			for (const link of node.targetLinks!.sort((a, b) => b.value - a.value)) {
				link.width = link.width ?? (link.value / (node.value || 1)) * nodeH;
		...
		g.selectAll(".left-label")
			...
			.attr("fill", "#55598a")
			.style("cursor", onNodeClick ? "pointer" : "default")
			.text((d) => (width < 500 && d.label.length > 12 ? `${d.label.slice(0, 12)}\u2026` : d.label))
			.on("mouseenter", function () {
				d3.select(this).attr("fill", "#ff8a2a");
			})
			.on("mouseleave", function () {
