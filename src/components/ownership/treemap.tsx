...
export function Treemap({ data, onCellClick }: TreemapProps) {
	...
	const draw = useCallback(() => {
		...
		d3.treemap<TreemapNode>().size([width, height]).padding(3).paddingInner(2).round(true)(root);
		...
		cells
			...
			.attr("fill", (d) => {
				...
				if (d.data.name === "Public Float") return "#e7e0d2";
				return type ? INVESTOR_TYPE_COLORS[type] : "#8b8fb0";
			})
			.attr("fill-opacity", (d) => (d.data.name === "Public Float" ? 0.7 : 0.85))
			.attr("stroke", (d) => {
				const type = d.data.type as InvestorType | undefined;
				if (d.data.name === "Public Float") return "#c9c0a7";
				return type ? INVESTOR_TYPE_COLORS[type] : "#8b8fb0";
			})
			.attr("stroke-opacity", 0.2)
			.attr("stroke-width", 1)
			.style("cursor", (d) => (d.data.name !== "Public Float" ? "pointer" : "default"))
			.on("mouseenter", function (event, d) {
				d3.select(this)
					.attr("fill-opacity", d.data.name === "Public Float" ? 0.5 : 0.95)
				...
				tooltip
					...
					.html(`<strong>${d.data.name}</strong><br/>${(d.data.value ?? 0).toFixed(2)}%`);
			})
			.on("mousemove", (event) => {
			...
			.on("mouseleave", function () {
				d3.select(this)
					.attr("fill-opacity", (d: any) => (d.data.name === "Public Float" ? 0.4 : 0.75))
					.attr("stroke-opacity", 0.2)
					.attr("stroke-width", 1);
				tooltip.style("display", "none");
			})
			.on("click", (_, d) => {
				if (d.data.name !== "Public Float") onCellClickRef.current?.(d.data.name);
			});
		...
		cells
			...
			.attr("fill", (d) => (d.data.name === "Public Float" ? "#55598a" : "#ffffff"))
		...
		cells
			...
			.attr("fill", (d) => (d.data.name === "Public Float" ? "#55598a" : "rgba(255,255,255,0.92)"))
			.attr("pointer-events", "none")
			.attr("font-family", "var(--font-mono, monospace)")
			.text((d: any) => {
				const w = d.x1 - d.x0;
	...
	return (
		<div ref={containerRef} className="relative" style={{ width: "100%", minWidth: 0 }}>
