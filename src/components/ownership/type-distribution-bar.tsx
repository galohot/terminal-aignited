	...
	useEffect(() => {
		...
		data.forEach((d, i) => {
			...
			g.append("text")
				...
				.attr("fill", "#55598a")
				.text(width < 400 && d.label.length > 8 ? `${d.label.slice(0, 8)}\u2026` : d.label);
			...
			g.append("rect")
				...
				.on("mouseenter", function (event) {
					d3.select(this).transition().duration(150).attr("y", y).attr("height", barHeight);
					tooltip
						.style("display", "block")
						...
						.html(
							`<strong>${d.label}</strong><br/>` +
								`<span style="color:#55598a">Holdings:</span> ${d.count.toLocaleString()}<br/>` +
								`<span style="color:#55598a">Total ownership:</span> ${d.totalPct.toFixed(1)}%<br/>` +
								`<span style="color:#55598a">Avg per holding:</span> ${d.avgPct.toFixed(1)}%`,
						);
				})
